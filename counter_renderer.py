...
def get_counter(n: int) -> str:
    ...
    """    
    return config["output"]["prepend"] + str(round((n / 10) + config["output"]["start_at"], 1))
...
class CounterRenderer:
    ...
    def __init__(self, keyframes: List[List[tuple]], **kwargs):
        ...
        self.c = RenderConfig(**{**config["output"], **kwargs}) # create the RenderConfig based on the config values which are overwritten by kwargs to this class
    ...
    def write_on_frame(self, frame: np.ndarray, text: str) -> np.ndarray:
        ...
        draw.text((0, 0), text, font=self.font, fill=(255, 255, 255, 0)) # Draw the text
        return np.array(img_pil)

    def render_clip(self, keyframes: List[tuple], i: int): # TODO: Split render_clip into multiple methods - too long
        ...
        frames = np.linspace(0, seconds, num=round(self.c.fps * seconds)) # create a numpy array with all the frame times needed 

        fourcc = cv2.VideoWriter_fourcc(*"MP42") # dont ask me why a MP42 fourcc is needed
        ...
        keyframe_index = 0
        for index, _ in enumerate(frames): # loop over all the frames, no need to get the actual value since we only need the next frame
            frame = np.zeros((self.c.height, self.c.width, 3), dtype=np.uint8) # create a frame with zeroes - black frame
            try:
                diff_next = (keyframes[keyframe_index] - min(keyframes)) - frames[index + 1] # calculate the difference between the relative keyframe time since start and the next frame time since start
                ...
                pass
            frame = self.write_on_frame(frame, get_counter(keyframe_index)) # write the counter on the frame
            video.write(frame) # and write the frame on the video

        for _ in range(self.c.fps * self.c.extend): # add fps * extend number of frames at the end
            frame = np.zeros((self.c.height, self.c.width, 3), dtype=np.uint8) # all filled with zeroes
            frame = self.write_on_frame(frame, get_counter(keyframe_index)) # and the end count
            video.write(frame)
